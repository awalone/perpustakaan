<?php
	...
	class Izin_iup_model extends CI_Model
	{
		...
		function get_all_registrasi($limit = NULL, $offset = NULL) {
			...
			$query = $this->db->query("select 
				...
				WHERE 
					a.id_jnsizin = '03' AND status_reg=0
		...
		function get_all_data_registrasi($id = NULL) {
			...
			$query = $this->db->query("select a.no_reg, b.nm_izin, c.nm_pemohon, d.nm_bdnusaha, d.atr_usaha 
				from
					tbl_registrasi a left join tbl_jnsizin b ON a.id_jnsizin = b.id_jnsizin left join tbl_pemohon c ON a.id_pemohon = c.id_pemohon left join tbl_bdnusaha d ON a.id_bdnusaha = d.id_bdnusaha WHERE a.id_jnsizin = '03' AND status_reg = 0 $where
			");
		...
		function get_all($limit=NULL,$offset=NULL)
		{
			...
			WHERE c.status_reg = 1 ORDER BY a.tgl_izin DESC
			LIMIT $offset, $limit
			");
			return $query;
		}
		...
		function get_all_by_id($id) {
			...
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha 
			WHERE
				c.status_reg = 1 AND (a.no_izin LIKE '%$id%' OR a.no_reko LIKE '%$id%' OR a.no_reg LIKE '%$id%' OR d.nm_pemohon LIKE '%$id%' OR e.nm_bdnusaha LIKE '%$id%')
		...
		##untuk menampilkan data perizinan yang sudah berstatus tercetak(2)
		function get_all_cetak($limit=NULL,$offset=NULL)
		{
			...
			LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha 
			WHERE c.status_reg = 2 OR c.status_reg = 3 ORDER BY a.tgl_izin DESC
			LIMIT $offset, $limit
			");
			return $query;
		}
		function get_all_cetak_by_id($id) {
			...
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha 
			WHERE
				(c.status_reg = 2 OR c.status_reg = 3) $where
			ORDER BY a.tgl_izin DESC");
			return $query;
		}
		...
		function get_data_izin_by_registrasi($id) {
			$query = $this->db->query("SELECT 
				a.no_izin, a.tgl_izin, a.no_reg, a.no_reko, a.tgl_reko, a.kekayaan, a.dagangan, a.id_lembaga,a.sta_izin, a.ket_izin,a.ket_dagang,
				b.nm_kel, 
				c.id_pemohon,c.status_reg,
				d.nm_pemohon,
				e.nm_bdnusaha
		...
		function get_data_by_id($id=NULL)
		{
			$query = $this->db->query("SELECT 
				...
				e.nm_bdnusaha,  e.npwp, e.jab_pengurus, e.nm_bdnusaha,e.tlp_bdnusaha,e.atr_usaha,e.id_bdnusaha,e.id_stausaha,e.id_katusaha,
				f.nm_lembaga,
				g.*
			FROM tbl_siup a 
				LEFT JOIN tbl_kelurahan b ON a.id_kel = b.id_kel
				...
				LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
				LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha
				LEFT JOIN tbl_lembaga f ON a.id_lembaga = f.id_lembaga
				LEFT JOIN tbl_instansi g ON kode = '00' 
			WHERE no_izin = '$id'
			");
			return $query->row();
		}
