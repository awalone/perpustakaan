<?php
	...
	class Izin_iui_model extends CI_Model
	{
		...
		function get_all_registrasi($limit = NULL, $offset = NULL) {
			...
			$query = $this->db->query("select 
				...
				left join tbl_bdnusaha d ON a.id_bdnusaha = d.id_bdnusaha WHERE 
					a.id_jnsizin = '05' AND status_reg=0 
		...
		function get_all_data_registrasi($id=NULL) {
			...
			$query = $this->db->query("select a.no_reg, b.nm_izin, c.nm_pemohon, d.nm_bdnusaha, d.atr_usaha 
				from
					tbl_registrasi a left join tbl_jnsizin b ON a.id_jnsizin = b.id_jnsizin left join tbl_pemohon c ON a.id_pemohon = c.id_pemohon left join tbl_bdnusaha d ON a.id_bdnusaha = d.id_bdnusaha WHERE a.id_jnsizin = '05' AND status_reg = 0
				$where
			");
		...
		function get_all($limit=NULL,$offset=NULL)
		{
			...
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha WHERE c.status_reg = 1
		...
		function get_all_by_id($id) {
			...
			WHERE
				c.status_reg = 1 AND (a.no_izin LIKE '%$id%' OR a.no_reg LIKE '%$id%' OR a.no_reko LIKE '%$id%' OR d.nm_pemohon LIKE '%$id%' OR e.nm_bdnusaha LIKE '%$id%')
			ORDER BY a.tgl_izin DESC");
			return $query;
		}
		...
		function get_all_data($id = NULL)
		{
			$where = "";
			if ($id != NULL) {
				$where .= "AND (a.no_reko LIKE '%$id%' OR no_izin LIKE '%$id%' OR d.nm_pemohon LIKE '%$id%' OR d.ktp_pemohon LIKE '%$id%')";
			}
			$query = $this->db->query("SELECT 
				a.no_izin, a.tgl_izin, a.no_reg, a.no_reko, a.tgl_reko, a.investasi, a.kap_prod, a.tenaker, 
				b.nm_kel, 
				c.id_pemohon,c.status_reg,c.id_jnsizin,
				d.nm_pemohon,
				e.nm_bdnusaha
			FROM 
				tbl_siui a 
			...
			LEFT JOIN tbl_registrasi c ON a.no_reg = c.no_reg
			LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha WHERE c.status_reg = 1 $where AND c.id_jnsizin = '05'");
			return $query->num_rows();
		}
		...
		function get_all_data_by_id($id) {
			...
			LEFT JOIN tbl_registrasi c ON a.no_reg = c.no_reg
			LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha WHERE (a.no_izin LIKE '%$id%' OR a.no_reko LIKE '%$id%' OR d.nm_pemohon LIKE '%$id%' OR d.ktp_pemohon LIKE '%$id%') AND c.id_jnsizin = '05'");
			return $query;
		}
		...
		#############################################
		## ******** UNntuk perizinan IUI yang sudah tercetak (2)
		#######################################################
		function get_all_cetak($limit=NULL,$offset=NULL)
		{
			...
			LEFT JOIN tbl_registrasi c ON a.no_reg = c.no_reg
			LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha WHERE c.status_reg = 2 OR c.status_reg = 3 ORDER BY a.tgl_izin DESC LIMIT $offset, $limit");
			return $query;
		}
		...
		function get_all_cetak_by_id($id) {
			...
			$query = $this->db->query("SELECT 
				a.no_izin, a.tgl_izin, a.no_reg, a.no_reko, a.tgl_reko, a.kekayaan, a.dagangan, 
				b.nm_kel, 
				c.id_pemohon,
				d.nm_pemohon,
				e.nm_bdnusaha
		...
		function get_all_data_cetak($id = NULL)
		{
			$where = "";
			if ($id != NULL) {
				$where .= "AND (a.no_izin LIKE '%$id%' OR d.nm_pemohon LIKE '%$id%' OR a.no_reko LIKE '%$id%' OR d.ktp_pemohon LIKE '%$id%')";
			}
			$query = $this->db->query("SELECT 
				a.no_izin, a.tgl_izin, a.no_reg, a.no_reko, a.tgl_reko, a.investasi, a.kap_prod, a.tenaker, 
				b.nm_kel, 
			...
			LEFT JOIN tbl_registrasi c ON a.no_reg = c.no_reg
			LEFT JOIN tbl_pemohon d ON c.id_pemohon = d.id_pemohon
			LEFT JOIN tbl_bdnusaha e ON c.id_bdnusaha = e.id_bdnusaha WHERE c.status_reg = 2 OR c.status_reg = 3 $where AND c.id_jnsizin = '05'");
			return $query->num_rows();
		}
		##################################################
		...
		function get_data_izin_by_registrasi($id) {
			$query = $this->db->query("SELECT 
				a.no_izin, a.tgl_izin, a.no_reg, a.no_reko, a.tgl_reko, a.investasi,kap_prod,tenaker, 
				b.nm_kel, 
				c.id_pemohon,c.status_reg,
				d.nm_pemohon,
				e.nm_bdnusaha
