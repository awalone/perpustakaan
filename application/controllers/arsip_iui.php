<?php
	...
	class Arsip_iui extends CI_Controller
	{	
		...
		function index($offset = 0)
		{
			if ($this->check() == TRUE) 
			{
				...
				$uri_segment = 3;
				$data['h2_title'] = 'Data Perizinan iui';
				$data['main_view'] = 'izin_iui/arsip_iui';
				...
				$data['statusArsip'] = site_url('arsip_iui/arsip_detail');
				$data['search']	= site_url('arsip_iui/search_process');
		...
		function view_arsip($id) {
			if ($this->check() == TRUE) 
			{
				...
				$data['main_view']	= 'izin_iui/arsip_view';
		...
		function arsip_detail($tipe,$id) {
			...
			{	
				$id = $this->enkrip->decode($id);
				$data['title'] = $this->title;
				$data['form_action'] = site_url('arsip_iui/add_process');
				$data['button'] = 'Upload Berkas';
				...
				$data['main_view']	= 'izin_iui/arsip_form';
		...
		function add_process() {
			...
			if ($this->check() == TRUE) 
			{
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$path_file = 'assets/uploads/arsip_iui';
					...
					//create path
					$this->libraryku->create_path($tipe_arsip,'iui',$tahun,$bulan,$tgl);
					...
					$path_file = "arsip/$type/iui/$tahun/$bulan/$tgl";
