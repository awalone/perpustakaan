<?php
	...
	class Arsip_sig extends CI_Controller
	{	
		...
		function index($offset = 0)
		{
			if ($this->check() == TRUE) 
			{
				...
				$uri_segment = 3;
				$data['h2_title'] = 'Data Perizinan sig';
				$data['main_view'] = 'izin_sig/arsip_sig';
				...
				$data['statusArsip'] = site_url('arsip_sig/arsip_detail');
				$data['search']	= site_url('arsip_sig/search_process');
		...
		function view_arsip($id) {	
			if ($this->check() == TRUE) 
			{
				...
				$data['main_view']	= 'izin_sig/arsip_view';
		...
		function arsip_detail($tipe,$id) {
			if ($this->check() == TRUE) 
			{
				$id = $this->enkrip->decode($id);
				$data['title'] = $this->title;
				$data['form_action'] = site_url('arsip_sig/add_process');
				$data['button'] = 'Upload Berkas';
				...
				$data['main_view']	= 'izin_sig/arsip_form';
		...
		function add_process() {
			...
			if ($this->check() == TRUE) 
			{
				...
				if ($this->form_validation->run() == TRUE) {
					...
					//create path
					$this->libraryku->create_path($tipe_arsip,'sig',$tahun,$bulan,$tgl);
					...
					$path_file = "arsip/$type/sig/$tahun/$bulan/$tgl";
