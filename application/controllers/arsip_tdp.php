<?php
	...
	class Arsip_tdp extends CI_Controller
	{	
		...
		function index($offset = 0)
		{
			if ($this->check() == TRUE) 
			{
				...
				$data['statusArsip'] = site_url('arsip_tdp/arsip_detail');
				$data['search']	= site_url('arsip_tdp/search_process');
		...
		function view_arsip($id) {
			if ($this->check() == TRUE) 
			{
				...
				$data['main_view']	= 'izin_tdp/arsip_view';
		...
		function arsip_detail($tipe,$id) {
			...
			{	
				$id = $this->enkrip->decode($id);
				$data['title'] = $this->title;
				$data['form_action'] = site_url('arsip_tdp/add_process');
				$data['button'] = 'Upload Berkas';
				...
				$data['main_view']	= 'izin_tdp/arsip_form';
		...
		function add_process() {
			...
			if ($this->check() == TRUE) 
			{
				...
				if ($this->form_validation->run() == TRUE) {
					...
					//create path
					$this->libraryku->create_path($tipe_arsip,'tdp',$tahun,$bulan,$tgl);
					...
					$path_file = "arsip/$type/tdp/$tahun/$bulan/$tgl";
					...
					if ($this->uploads->do_upload($path_file, $namaFile) == TRUE) {
						...
						if ($jumlahArsip > 1) {
							...
							//cek untuk mengirim sms ke nomor hp pemohon
